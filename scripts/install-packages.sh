...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    brew update && brew install git cmake zlib openssl curl
    status=$?
    ...
    sudo apt-get update && sudo apt-get install git cmake gcc g++ zlib1g-dev libssl-dev libcurl4-openssl-dev
    status=$?
